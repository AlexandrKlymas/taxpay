...
use EvolutionCMS\Main\Modules\OrdersArchive\OrdersModuleHelper;
...
class MainController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $services = SiteContent::active()->whereIn('template', [5,11])->get()->toArray();
        ...
        return View::make('Modules.OrdersArchive::index', array_merge($this->viewData, $data))->render();
    }
    ...
    public function loadOrders(Request $request)
    {
        ...
        $q->whereIn('service_orders.status',['wait','error','failure']);
        $q->where('service_orders.created_at','<',date('Y-m-d H:i:s',strtotime('-1 day')));
