...
//        if ($request->has('sortorder') && $this->session->get('sortorder')) $this->session->set(['sortorder' => $request->get('sortorder')]);
    ...
    public function pay(): string
    {
        $program = $this->session->get('program');
        if (empty($program)) {
            return $this->render->renderPayError(['error'=>'Вибачте, сервіс тимчасово недоступний. Спробуйте, будь ласка, пізніше.']);
...
//            evo()->sendRedirect(\UrlProcessor::makeUrl(124));
            ...
            //todo switch sandbox
        $liqPay = new \LiqPay($merchant['public'], $merchant['secret']);
//        $liqPay = new \LiqPay('sandbox_i45832017380', 'sandbox_bGvkHobyUw0BQkpvEN82hfHpTKkeNN8qMuXOpqJy');
        ...
        $paymentParams = [
            ...
            'version' => '3',
            'language' => 'uk',
            'result_url' => $baseUrl . '/pay-osago-check?contract='.$payment['contractId'],
    ...
    public function payCheck(Request $request)
    {
        ...
        if(!empty($request['data'])){
            $this->payCallback($request);
        }
        if (!empty($_GET['contract'])) {
            $order = OsagoOrders::where('contract_id', $_GET['contract'])
                ->first();
            if (!empty($order)) {
...
//        dd('no contract_id');
    ...
    {
        $order = OsagoOrders::where('contract_id',$request['contract'])
            ->first();
        if(empty($order)){
            evo()->logEvent(1,2,json_encode($request->toArray()),'Автоцывилка заказ не найден');
            return false;
        }
        ...
        if(empty($companyId)){
            evo()->logEvent(1,2,json_encode($request->toArray()),'Автоцывилка контракт не найден');
            return false;
        }
        ...
        if(empty($merchant)){
            evo()->logEvent(1,2,json_encode($request->toArray()),'Автоцывилка мерчант не найден');
            return false;
        }
    ...
    public function payCallback(Request $request)
    {
        ...
        if(!$this->checkSignature($request)){
            evo()->logEvent(1,2,json_encode($request->toArray()),'Автоцывилка ошибка сигнатуры');
            $data = [];
        }
